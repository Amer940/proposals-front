...
export function MoneyChart({
  ...
  chartDataYearly: yearlyAnalytics[];
}) {
  const [activeChart, setActiveChart] =
    React.useState<keyof typeof chartConfig>("months");

  const total = React.useMemo(
    () => ({
      months: chartData.reduce((acc, curr) => acc + +curr.months, 0),
      year: chartDataYearly.reduce((acc, curr) => acc + +curr.year, 0),
    }),
    []
  );
    ...
    <Card className="py-0">
      ...
      <CardContent className="px-2 sm:p-6">
        ...
        >
          <BarChart
            accessibilityLayer
            data={activeChart === "months" ? chartData : chartDataYearly}
            margin={{
              left: 12,
              right: 12,
            }}
