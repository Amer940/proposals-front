...
export function PartnerMainTable<TData, TValue>({
  ...
  const [nameFilter, setNameFilter] = useDebouncedState("", 750);
  const [countryFilter, setCountryFilter] = useDebouncedState("", 750);
  const [cityFilter, setCityFilter] = useDebouncedState("", 750);
  ...
  const [errorMessage, setErrorMessage] = useState(
    "Error happened while getting partners"
  );

  const { data, rowCount, loading, error, refetch } = useServerPartnerTableData(
    {
      pageIndex,
      pageSize,
      nameFilter,
